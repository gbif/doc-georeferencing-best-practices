...
Data that have been incorporated into the database and <<georeference,georeferenced>> need to be maintained and checked for <<data quality,quality>>. The quality checking process involves a number of steps, including receiving feedback from users, providing feedback to collectors, and running various validation tests. For more information on data quality and what it means for primary species collection data, see https://doi.org/10.15468/doc-jrgg-a190[Chapman 2005b^]. Two major principles associated with data quality and data cleaning are:
...
* Georeferencing Quick Reference Guide ({gqg}[Zermoglio et al. 2020])
* Georeferencing Calculator ({gcm}[Wieczorek & Wieczorek 2020])
* Geographic concepts
