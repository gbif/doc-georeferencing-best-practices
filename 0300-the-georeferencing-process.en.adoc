...
* Train participating contributors and ¤<<georeference,georeferencing>>¤ operators (see <<__*6.3.1 Data Entry*__>> and <<Training>>).
...
If any of the ¤<<locality clause,clauses>>¤ in the ¤<<locality>>¤ description is classified as one of the three ¤<<locality>>¤ types, ‘_dubious_’, ‘_cannot be located_’, or ‘_demonstrably inaccurate_’, then the ¤<<locality>>¤ should not be ¤<<georeference,georeferenced>>¤. Instead, an annotation should be made to the ¤<<locality>>¤ record giving the reason why it is not being ¤<<georeference,georeferenced>>¤. See also <<__*2.4 Difficult Localities*__>> in Zermoglio _et al._ (2020).
