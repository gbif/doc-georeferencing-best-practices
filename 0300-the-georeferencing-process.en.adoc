...
Elevation can often be used as a constraint to distinguish between two similarly named localities or to refine the uncertainty in a georeference. If both maximum and minimum elevations are given, then the contours may be used to refine a locality and its uncertainty as described in {gqg}#image-mountain-boundaries[Figure 5^] in {gqg}[Georeferencing Quick Reference Guide (Zermoglio et al. 2020)^].
...
where x=dcos(θ), y=dsin(θ), x′=dcos(θ’), and y′=dsin(θ′). and the uncertainty in the above example would be 3.90 km.
