...
Location-related fields to consider for <<georeference,georeferencing>> include all of the geography, <<locality>>, <<elevation>>, <<depth>>, and <<georeference>> terms in the *Location* class of <<Darwin Core>> (see term:dwc[Location] and <<Mapping to Darwin Core>>) as well as the following fields that can have an influence on the <<georeference>>:
