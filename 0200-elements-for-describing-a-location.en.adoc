...
This type of ¤<<locality>>¤ refers to rectilinear distances in two orthogonal ¤<<direction,directions>>¤ from a ¤<<feature>>¤, for example, "2 mi E and 1.5 mi N of Kandy" (see <<__*2.2.4 Offset - Distance along Orthogonal Directions*__>> in Georeferencing Q__*uick Reference Guide *__(Zermoglio _et al._ 2020) and <<Figure 12>>. This way of describing a locality can be very effective, as it tends to remove one of the potentially largest sources of *uncertainty*, the ever-expanding ¤<<uncertainty>>¤ of ¤<<direction>>¤ with distance. Using orthogonal ¤<<direction,directions>>¤ removes all ¤<<direction,directional>>¤ ¤<<uncertainty>>¤, as orthogonality implies directly in the orthogonal ¤<<direction,directions>>¤ "by air". It is for this reason that this ¤<<locality type>>¤ is highly recommended for ¤<<locality>>¤ descriptions.
