...
One of the greatest impediments to effective collaborative <<georeference,georeferencing>>, is the absence of tools to easily <<repatriate>> the <<georeference,georeferenced>> information back to the data sources (Barkwell and Murrell 2012, Grant _et al._ 2018). A number of projects are working on this, especially GEOLocate (see, <<GEOLocate>>) in conjunction with the Symbiota platform (Gries _et al._ 2014). It is hoped that this document will provide consistency of methodology and documentation and lead to more collaborative <<georeference,georeferencing>>.
