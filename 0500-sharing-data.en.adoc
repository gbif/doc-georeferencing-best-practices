...
As recommended elsewhere in this document, <<georeference,georeferences>> should be recorded and stored at the best possible resolution and <<precision>>. If, however, the <<location>> of a taxon is regarded as sensitive for some reason following the guidelines as set out in https://doi.org/10.15468/doc-5jp4-5g10[Chapman 2020^] and https://doi.org/10.15468/doc-b02jgt10[Chapman & Grafton 2008^], and it is agreed that the detailed location information should not be shared, we recommend, that the data only be <<generalization,generalized>> at the time of sharing or publishing of the data.

We recommend that if data are to be generalized>> that it be done by reducing the number of decimal places (for example when using <<decimal degrees>>) at which the data are published (https://doi.org/10.15468/doc-b02jgt10[Chapman & Grafton 2008^], https://doi.org/10.15468/doc-5jp4-5g10[Chapman 2020^]). Good practice dictates that whatever you do to generalize the data, it be documented so that users of the data know what reliance can be placed on them. As far as the generalization of georeferencing data is concerned it is important to record that the data have been generalized using a ‘_decimal geographic grid_’, and record both:
